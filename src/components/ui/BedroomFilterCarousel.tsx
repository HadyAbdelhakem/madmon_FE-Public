...
interface BedroomFilterCarouselProps {
  onBedroomSelect: (bedroom: number | string) => void; // New prop for callback
}


const BedroomFilterCarousel: React.FC<BedroomFilterCarouselProps> = ({ onBedroomSelect }) => {
  const [selectedBedroom, setSelectedBedroom] = useState<number | string>(1);
  const bedroomOptions = Array.from({ length: 10 }, (_, i) => i + 1);

  const handleBedroomClick = (bedroom: number | string) => {
    setSelectedBedroom(bedroom);
    onBedroomSelect(bedroom); // Call the callback with the selected bedroom
  };
      ...
      >
        {bedroomOptions.map((num) => (
          <NumberButtonFilter
            key={num}
            number={num}
            selected={selectedBedroom === num}
            onClick={() => handleBedroomClick(num)}          />
        ))}
        <NumberButtonFilter
          number="+10"
          selected={selectedBedroom === "+10"}
          onClick={() => handleBedroomClick("+10")}        />
      </Carousel>
    </Box>
  );
};

export default BedroomFilterCarousel;
